...
export async function POST(_req: NextRequest, ctx: Ctx) {
  ...
  // 3) Update line as to_create and clear item_id
  const { error: e3 } = await supabase
    .from('invoice_lines')
    .update({ item_id: null, match_state: 'to_create' })
    .eq('line_id', lineId)
  if (e3) return NextResponse.json({ ok: false, error: e3.message }, { status: 500 })

  // 4) Ensure draft has item_id=null at that index
